...
class NERPDataSetReader():
    """Generic NERP DataSetReader"""
    ...
    def __getitem__(self, item):
        ...
        if len(tokens) > self.max_len-2:
            logger.warning(
                f'Sentence #{item} length {len(tokens)} exceeds max_len {self.max_len} and has been truncated')
        ...
        input_ids = [self.transformer_tokenizer.cls_token_id] + \
            input_ids + [self.transformer_tokenizer.sep_token_id]

        # fill out other inputs for model.
        target_tags = [self.tag_outside_transformed] + \
            target_tags + [self.tag_outside_transformed]
        ...
        if self.pad_sequences:
            ...
            target_tags = target_tags + \
                ([self.tag_outside_transformed] * padding_len)
...
def create_dataloader(sentences,
    ...
    if not pad_sequences and batch_size > 1:
        logger.info(
            "setting pad_sequences to True, because batch_size is more than one.")
        pad_sequences = True

    data_reader = NERPDataSetReader(
        sentences=sentences,
        tags=tags,
