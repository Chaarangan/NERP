...
class CRF(nn.Module):
    ...
    def __init__(
        self, num_labels: int, pad_idx: Optional[int] = None, device: str = "cpu"
    ) -> None:
        """
        ...
        self.trans_matrix = nn.Parameter(torch.empty(num_labels, num_labels)).to(device)
        # transition matrix of start and end settings
        self.start_trans = nn.Parameter(torch.empty(num_labels)).to(device)
        self.end_trans = nn.Parameter(torch.empty(num_labels)).to(device)
    ...
    def viterbi_decode(self, h: FloatTensor, mask: BoolTensor) -> List[List[int]]:
        ...
        for t in range(1, seq_len):
            ...
            previous_score = score[t - 1].view(batch_size, self.num_labels, 1)
